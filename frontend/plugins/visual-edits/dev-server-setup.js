...
function setupDevServer(config) {
  config.setupMiddlewares = (middlewares, devServer) => {
    ...
    const isAllowedOrigin = (origin) => {
      ...
      // Allow all yuvraj.dev subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*yuvraj\.dev$/)) {
        return true;
      }

      // Allow all yuvraj.com subdomains
      if (origin.match(/^https:\/\/([a-zA-Z0-9-]+\.)*yuvraj\.com$/)) {
        return true;
      }
      ...
      try {
        ...
        Object.entries(changesByFile).forEach(([fileName, fileChanges]) => {
          ...
          try {
            ...
            execSync(`git -c user.name="visual-edit" -c user.email="support@yuvraj.dev" add "${targetFile}"`);
            execSync(`git -c user.name="visual-edit" -c user.email="support@yuvraj.dev" commit -m "visual_edit_${timestamp}"`);
          } catch (gitError) {
